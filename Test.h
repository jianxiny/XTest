...
namespace MereTDD {
    ...
    public:
        explicit ConfirmException(int line)
                : mLine(line) {}
    ...
    public:
        explicit MissingException(std::string_view exType)
                : mExType(exType) {}

        [[nodiscard]] std::string_view exType() const {
            return mExType;
        }
    ...
    inline void addTest(std::string_view suitName, Test *test) {
        std::string name(suitName);
    ...
    public:
        ...
        [[nodiscard]] int confirmLocation() const {
    ...
    public:
        ...
        std::string_view expectedReason() const {
            return mExceptedReason;
        }

        void setExpectedFailureReason(std::string_view reason) {
            mExceptedReason = reason;
        }

    private:
        std::string mExceptedReason;
    };
...
{ \
public: \
    MERETDD_CLASS (std::string_view name, std::string_view suite) \
    : Test(name, suite) \
    { } \
    void run () override; \
}; \
